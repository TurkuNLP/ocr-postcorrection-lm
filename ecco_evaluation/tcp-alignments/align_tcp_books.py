...
def align(from_direction, alignment_file, output_fname):
    ...
    with open(tcp_books, "rt", encoding="utf-8") as f:
        for line in tqdm.tqdm(f, desc="Reading TCP books"):
            ...
            reference_text = compact(book["tcp reference"])

            #if "\u0085" in reference_text or "\u0085" in original_text or "\u0085" in corrected_text:
            #    print(f"\nFound \\u0085 in {url}", file=sys.stderr)
            #    print("Reference:", "\u0085" in reference_text, file=sys.stderr)
            #    print("Original:", "\u0085" in original_text, file=sys.stderr)
            #    print("Corrected:", "\u0085" in corrected_text, file=sys.stderr)

            if from_direction == "original":
                from_text = compact(original_text)
            elif from_direction == "correction":
                from_text = compact(corrected_text)
            else:
            ...
            for from_index, reference_index in zip(alignment[0], alignment[1]):
                ...
                aligned_characters += from_end-from_start + reference_end-reference_start
                ...
                if reference_char_index < reference_start:
                    chars = reference_text[reference_char_index:reference_start]
                ...
                if from_char_index < from_start:
                    chars = from_text[from_char_index:from_start]
                ...
                a_reference = reference_text[reference_start:reference_end]
                a_from = from_text[from_start:from_end]
                ...
                assert len(a_reference) == len(a_from), f"Reference: {a_reference} ({len(a_reference)}), From: {a_from} ({len(a_from)})"
                ...
                # debug, print everything so far
                #if len(a_original) != len(a_corrected):
                #    print("Failed to process book", url, "skipping.")
                #    fail = True
                #    break
                    #print(f"Original: {a_original} ({len(a_original)}), Corrected: {a_corrected} ({len(a_corrected)})")
                    #for i in range(len(final_original)-100, len(final_original), 100):
                    #    print()
            ...
            if (len(reference_text[reference_char_index:]) + len(from_text[from_char_index:]) > 200) or (aligned_characters / (len(reference_text) + len(from_text)) < 0.9):
                print("Book:", url)
                print(f"Aligned characters: {aligned_characters} / ({len(reference_text) + len(from_text)}) {aligned_characters/len(reference_text)+len(from_text)*100:.2f}%")
                ...
                print(f"Suspicious final alignment block:", final_alignments[-200:].count("|") < 50)
            ...
            if len(reference_text[reference_char_index:]) > 0:
                remaining_chars = reference_text[reference_char_index:]
                for c in remaining_chars:
                    final_reference += c
                    final_from += " " # space for unaligned part
                    final_alignments += "-" # deletion
            if len(from_text[from_char_index:]) > 0:
                remaining_chars = from_text[from_char_index:]
                for c in remaining_chars:
                    final_from += c
                    final_reference += " " # space for unaligned part
                    final_alignments += "+" # insertion
            ...
            assert reference_text == compact(final_reference), "Reference text mismatch"
            assert from_text == compact(final_from), f"{from_direction} text mismatch"

            #print("original-reference2.zip", file=sys.stderr)
            ...
            #if url == "0373500101":
            #    print("\nBOOK:", url)
            #    for i in range(0, len(final_reference), 100):
            #        print(final_reference[i:i+100])
            #        print(final_from[i:i+100])
            #        print(final_alignments[i:i+100])
            #        print()
                
            # for each book, print the last 500 characters with debug prints
            #for i in range(len(final_reference)-500, len(final_reference), 100):
            #    print("\nBOOK:", url)
            #    print(final_reference[i:i+100])
            #    print(final_from[i:i+100])
            #    print(final_alignments[i:i+100])
    ...
    print("\nSuspicious books:")
    print(suspicious_books)
    print(len(suspicious_books))
...
if __name__ == "__main__":
    ...
    align(args.from_direction, args.alignment_file, args.output_file)
